...
namespace MyAspx.Framework.WebUnit.Common
{
    ...
    public class Utils
    {
        ...
        {
            return Regexlib.IsNumeric(Expression);
        }
        ...
        {
            return Regexlib.IsDouble(Expression);
        }
        ...
        {
            return Regexlib.IsNumericArray(strNumber);
        }
        ...
        public static string GetFullUrl(string url, string sUrlTemp)
        {
            if (url.ToLower().StartsWith("http") == true)
            { }
            else if (url.StartsWith("/") == true)
            {
                Uri u = new Uri(sUrlTemp);
                string sHost = u.Host;
                url = "http://" + sHost + url;
            }
            else
            {
                url = sUrlTemp.Substring(0, sUrlTemp.LastIndexOf("/") + 1) + url;
            }
            return url;
        }
        public static string OutHtml(string url, string Encode,bool isReplare=false)
        {
            string strRet = null;
            ...
            try
            {
                ...
                StreamReader ser = new StreamReader(sr, System.Text.Encoding.GetEncoding(Encode));
        ...
        public static string[] GetHtmlAList(string sHtmlText)
        {
            // 定义正则表达式用来匹配 a 标签
            Regex regImg = new Regex(@"<a\b[^<>]*?\bhref[\s\t\r\n]*=[\s\t\r\n]*[""']?[\s\t\r\n]*(?<Url>[^\s\t\r\n""'<>]*)[^<>]*?/?[\s\t\r\n]*>(?<Title>.*?)</a>", RegexOptions.IgnoreCase);
            ...
            //string[] sUrlList = new string[100];
            // 取得匹配项列表
            foreach (Match match in matches)
            {
                //if (i < 99)
                //{
                sUrlList[i] = match.Value + "卐" + match.Groups["Url"].Value + "卐" + match.Groups["Title"].Value;
                i++;
                //}
            }
            return sUrlList;
        ...
        public static string[] GetHtmlImageUrlList(string sHtmlText)
        {
            ...
            // 取得匹配项列表
            foreach (Match match in matches)
                sUrlList[i++] = match.Value + "卐" + match.Groups["imgUrl"].Value;
        ...
        /// <param name="sHtmlText">HTML代码</param>
        /// <returns>图片的URL列表</returns>
        public static string[] GetHtmlImageUrlList(string sHtmlText, string attrname)
        {
            // 定义正则表达式用来匹配 img 标签
            Regex regImg = new Regex(@"<img\b[^<>]*?\b" + attrname + @"[\s\t\r\n]*=[\s\t\r\n]*[""']?[\s\t\r\n]*(?<imgUrl>[^\s\t\r\n""'<>]*)[^<>]*?/?[\s\t\r\n]*>", RegexOptions.IgnoreCase);
        ...
        public static string GetValueByRule(string sFangDetailHtml, string sRules, string sGroupsName)
        {
            string str = "";
            try
            {
                Regex myRegex = new Regex(sRules.ToLower(), RegexOptions.IgnoreCase | RegexOptions.Multiline);

                Match matchMade = myRegex.Match(sFangDetailHtml);
                if (matchMade.Success)
                {
                    str = matchMade.Groups[sGroupsName.ToLower()].Value.ToString();
        ...
        public static string GetValueByRule(string sFangDetailHtml, string sRules, string sGroupsName, int k)
        {
            string str = "";
            Regex myRegex = new Regex(sRules.ToLower(), RegexOptions.IgnoreCase | RegexOptions.Multiline);
            int i = 0;
