    ...
    public class HttpHelper
    {
        ...
        private void SetCer(HttpItem objhttpItem)
        {
            if (!string.IsNullOrEmpty(objhttpItem.CerPath))
            {
                ...
                //将证书添加到请求里
                request.ClientCertificates.Add(new X509Certificate(objhttpItem.CerPath, objhttpItem.CerPWD));
            }
            else
                //初始化对像，并设置请求的URL地址
                request = (HttpWebRequest)WebRequest.Create(objhttpItem.URL);
        ...
        private void SetCookie(HttpItem objhttpItem)
        {
            ...
            if (objhttpItem.CookieContainer != null)
            {
                request.CookieContainer = objhttpItem.CookieContainer;
            }
        }
        /// <summary>
        /// 设置Post数据
